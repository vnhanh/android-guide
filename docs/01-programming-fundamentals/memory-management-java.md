---
id: fundamentals-memory-management-java
title: Memory Management in Java — Traced GC & the Same Longer-Lived-Holder Leak
description: Why Java's traced garbage collector only frees what's unreachable, the singleton-holds-Activity/Context leak pattern, and the team mechanism that catches it before it ships.
tags: [memory-management, garbage-collection, java, mid, senior, lead]
lang: en
status: complete
...
prerequisites: []
outcomes:
  - "Explain what actually determines when a Java object gets freed"
...
# Memory Management in Java — Traced GC & the Same Longer-Lived-Holder Leak
...
collection**. Java is the original librarian this model is named after.

## Mid {concept=memory-management/model}

**Interview question: "In Java, what actually determines when an object gets freed?"**

```java
// Java — traced GC. An object is freed once nothing reachable from a
...
it is logically dead and nobody will ever use it again.
...
class AnalyticsManager {
    ...
    void setListener(EventListener l) { this.listener = l; }
}

class ProfileActivity extends Activity {
    @Override
    protected void onCreate(Bundle savedInstanceState) {
        // This anonymous class captures `ProfileActivity.this` implicitly — AnalyticsManager
        // now holds the Activity for as long as the process lives, long past onDestroy.
        AnalyticsManager.getInstance().setListener(event -> updateUi(event));
    }
}
```
...
class ProfileActivity extends Activity {
    ...
    protected void onDestroy() {
        AnalyticsManager.getInstance().setListener(null); // break the reference before this Activity dies
        super.onDestroy();
    }
}
```

**Follow-up:** "How does this compare to Swift's memory model?" Java, Kotlin, Dart and
...
Naming the mechanism, in order of how automatically it runs: a static analysis rule catches the
pattern before a human ever looks — Android lint has rules that flag a `Context` or `View`
reference held past its expected lifetime. Short of a lint rule, a code-review convention that is
actually checkable: every listener registration call must show its paired removal in the same
review, so a reviewer rejects the diff on sight rather than trusting the author remembered. For a
critical, high-traffic screen where a leak would degrade the whole app's memory footprint, a
CI-gated memory benchmark or a dedicated leak-detection tool — LeakCanary is the standard example —
running automatically rather than relying on someone manually opening the profiler.
